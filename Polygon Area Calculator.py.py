...
def create_spend_chart(categories):
    ...
    for category in categories:
        total_withdraw = sum(-entry["amount"] for entry in category.ledger if entry["amount"] < 0)
        spent.append(total_withdraw)

    total_spent = sum(spent)
    percentages = [int((amount / total_spent) * 10) * 10 for amount in spent]  # rounded down to nearest 10
